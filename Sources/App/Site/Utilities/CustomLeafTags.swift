...
/// Usage: #formatGroupText(String) -> String
...
struct FormatPostTextTag: UnsafeUnescapedLeafTag {
	...
	func render(_ ctx: LeafContext) throws -> LeafData {
		...
		words = words.map {
			if $0.hasPrefix("@") && $0.count <= 50 && $0.count >= 3 {
						...
						.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed)
				{
					...
					case .group: searchHref = "/tweets?hashtag=\(hashtag)"
		...
		// e.g. "http://192.168.0.19:8081/group/ADDBA5D9-1154-4033-88AE-07B12F3AE162"
...
/// is currently no functional difference between EvenTimeTag and GroupTimeTag. It is being left in
...
///
/// Usage in Leaf templates:: #groupTime(startTime, endTime) -> String
struct GroupTimeTag: LeafTag {
	func render(_ ctx: LeafContext) throws -> LeafData {
		try ctx.requireParameterCount(2)
		guard let startTimeDouble = ctx.parameters[0].double, let endTimeDouble = ctx.parameters[1].double else {
			throw "Leaf: Unable to convert parameter to double for date"
