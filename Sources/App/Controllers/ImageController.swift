...
struct ImageController: APIRouteCollection {
	...
	///
	/// Returns a user-created image previously uploaded to the server. This includes images in Twitarr posts, ForumPosts, GroupPosts, User Avatars, and Daily Theme images.
	...
	/// GroupPosts, User Avatars, and Daily Theme images. The exact size of the thumbnail may vary based on the usage given at upload time.
	///
	/// Image filenames should have a form of: `UUIDString.typeExtension` where the UUIDString matches the output of `UUID().string` and `typeExtension`
	/// matches one of : "bmp", "gif", "jpg", "png", "tiff", "wbmp", "webp". Example: `F818D809-AAB9-4C92-8AAD-6AE483C8AB82.jpg`. The `thumb` and `full`
	/// versions of this call return differently-sized versions of the same image when called with the same filename.
	...
	/// ForumPosts, GroupPosts, User Avatars, and Daily Theme images. Archived images are only accessible by Moderators and above.
	///
	/// Image filenames should have a form of: `UUIDString.typeExtension` where the UUIDString matches the output of `UUID().string` and `typeExtension`
	/// matches one of : "bmp", "gif", "jpg", "png", "tiff", "wbmp", "webp". Example: `F818D809-AAB9-4C92-8AAD-6AE483C8AB82.jpg`. The `thumb` and `full`
	/// versions of this call return differently-sized versions of the same image when called with the same filename.
...
public struct Xoshiro: RandomNumberGenerator {
	...
	public init(seed: UUID) {
		let (
			byte0, byte1, byte2, byte3, byte4, byte5, byte6, byte7, byte8, byte9, byteA, byteB, byteC, byteD, byteE,
			byteF
		) = seed.uuid
		var high64: UInt64 =
			(UInt64(byte0) << 0o70) | (UInt64(byte1) << 0o60) | (UInt64(byte2) << 0o50) | (UInt64(byte3) << 0o40)
		high64 |= (UInt64(byte4) << 0o30) | (UInt64(byte5) << 0o20) | (UInt64(byte6) << 0o10) | (UInt64(byte7) << 0o00)
		var low64: UInt64 =
			(UInt64(byte8) << 0o70) | (UInt64(byte9) << 0o60) | (UInt64(byteA) << 0o50) | (UInt64(byteB) << 0o40)
		low64 |= (UInt64(byteC) << 0o30) | (UInt64(byteD) << 0o20) | (UInt64(byteE) << 0o10) | (UInt64(byteF) << 0o00)
		self.init(seed: (0xf4ea_82ac_c99f_4b28, 0xd72f_22e3_59f3_c6ad, high64, low64))
	}
