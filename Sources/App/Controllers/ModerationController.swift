...
struct ModerationController: APIRouteCollection {
	...
	func moderatorActionLogHandler(_ req: Request) async throws -> [ModeratorActionLogData] {
		...
		let result = try await ModeratorAction.query(on: req.db).range(start..<(start + limit)).sort(\.$createdAt, .descending)
	...
	func twarrtModerationHandler(_ req: Request) async throws -> TwarrtModerationData {
		...
		let reports = try await Report.query(on: req.db).filter(\.$reportType == .twarrt).filter(\.$reportedID == paramVal)
	...
	func twarrtSetModerationStateHandler(_ req: Request) async throws -> HTTPStatus {
		...
		await twarrt.logIfModeratorAction(ModeratorActionType.setFromModerationStatus(twarrt.moderationStatus), user: user, on: req)
	...
	func forumPostSetModerationStateHandler(_ req: Request) async throws -> HTTPStatus {
		...
		await forumPost.logIfModeratorAction(ModeratorActionType.setFromModerationStatus(forumPost.moderationStatus), user: user, on: req)
	...
	func forumSetModerationStateHandler(_ req: Request) async throws -> HTTPStatus {
		...
		await forum.logIfModeratorAction(ModeratorActionType.setFromModerationStatus(forum.moderationStatus), user: user, on: req)
	...
	func fezModerationHandler(_ req: Request) async throws -> FezModerationData {
		...
		let reportData = try reports.map { try ReportModerationData.init(req: req, report: $0) }
		let modData = FezModerationData(fez: fezData, isDeleted: lfg.deletedAt != nil, 
				moderationStatus: lfg.moderationStatus, edits: editData, reports: reportData)
		return modData
	}
	...
	func fezSetModerationStateHandler(_ req: Request) async throws -> HTTPStatus {
		...
		await lfg.logIfModeratorAction(ModeratorActionType.setFromModerationStatus(lfg.moderationStatus), user: user, on: req)
	...
	func fezPostModerationHandler(_ req: Request) async throws -> FezPostModerationData {
		...
		let reportData = try reports.map { try ReportModerationData.init(req: req, report: $0) }
		let modData = FezPostModerationData(fezPost: fezPostData, fezID: lfgPost.$fez.id, isDeleted: lfgPost.deletedAt != nil, 
				moderationStatus: lfgPost.moderationStatus, reports: reportData)
		return modData
	}
	...
	func fezPostSetModerationStateHandler(_ req: Request) async throws -> HTTPStatus {
		...
		await lfgPost.logIfModeratorAction(ModeratorActionType.setFromModerationStatus(lfgPost.moderationStatus), user: user, on: req)
	...
	func profileModerationHandler(_ req: Request) async throws -> ProfileModerationData {
		...
		guard let targetUser = try await User.query(on: req.db).filter(\.$id == targetUserID).withDeleted().first() else {
	...
	func profileSetModerationStateHandler(_ req: Request) async throws -> HTTPStatus {
		...
		await targetUser.logIfModeratorAction(ModeratorActionType.setFromModerationStatus(targetUser.moderationStatus), user: user, on: req)
	...
	func userModerationHandler(_ req: Request) async throws -> UserModerationData {
		...
		let reports = try await Report.query(on: req.db).filter(\.$reportedUser.$id ~~ allUserIDs).sort(\.$createdAt, .descending).all()
		let reportData = try reports.map { try ReportModerationData.init(req: req, report: $0) }
		let modData = try UserModerationData(user: allAccounts[0], subAccounts: Array(allAccounts.dropFirst()), reports: reportData)
		return modData
	}
	...
	func applyUserTempQuarantine(_ req: Request) async throws -> HTTPStatus {
		...
		let targetUser = try await User.findFromParameter(userIDParam, on: req)
		guard targetUser.accessLevel < UserAccessLevel.moderator, targetUser.accessLevel != UserAccessLevel.client else {
