...
protocol ImageHandler {
    ...
    func processImage(data: Data?, forType: ImageHandlerType, on req: Request) throws -> EventLoopFuture<String>
}
...
extension ImageHandler {
    ...
    func processImage(data: Data?, forType: ImageHandlerType, on req: Request) -> EventLoopFuture<String> {
		do {
			...
			let baseDir = DirectoryConfiguration.detect().workingDirectory.appending(imageDir)
			...
			return req.eventLoop.future(name)
		}
		catch {
...
extension ImageHandler {
    ...
    func archiveImage(_ image: String, from imageDir: String) -> Void {
        ...
        let basePath = DirectoryConfiguration.detect().workingDirectory.appending(imageDir)
