		...
		do {
			...
			for jsonGame in gamesList {
				if let expands = jsonGame.expands {
					guard let basegame = try await Boardgame.query(on: database).filter(\.$gameName == expands).first() else {
						database.logger.log(level: .notice, 
								"Could not find basegame \"\(expands)\" to attach game expansion \"\(jsonGame.gameName)\"")
						continue
					}
					guard let expansion = try await Boardgame.query(on: database).filter(\.$gameName == jsonGame.gameName).first() else {
						database.logger.log(level: .notice, 
