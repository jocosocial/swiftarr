...
// RCF: Copied from gd.c. Defined in gd_errors.h.
...
// RCF: Added these functions that do the actual retrieving/saving of exif data
uint32_t readFourByteExifData(unsigned char *ptr, int bigEndian)
{
	uint32_t result = bigEndian ?
...
uint16_t readTwoByteExifData(unsigned char *ptr, int bigEndian)
{
	uint16_t result = bigEndian ?
			(uint16_t) ptr[0] <<  8 | (uint16_t) ptr[1] :
			(uint16_t) ptr[0]       | (uint16_t) ptr[1] <<  8; 
	return result;
}

// Actual hackishness: Stores orientation result in the "polyAllocated" field of the gdImagePtr. 
// That field is definitely not for this purpose.
void process_exif_markers(j_decompress_ptr cinfo, gdImagePtr image)
{
	jpeg_saved_marker_ptr markerStruct = cinfo->marker_list;
	while (markerStruct != NULL && markerStruct->marker != JPEG_APP0 + 1) 
	{
		markerStruct = markerStruct->next;
	}
	if (markerStruct == NULL) return;
	unsigned int len = markerStruct-> data_length;
	if (len < 28) return;
	unsigned char *data = markerStruct->data;
	unsigned char exifHdr[] = { 0x45, 0x78, 0x69, 0x66, 0x00, 0x00 };
	for (int index = 0; index < 6; ++index)
	{
		if (data[index] != exifHdr[index]) return;
	}

	int bigEndian = data[6] == 0x4D ? 1 : 0;
	unsigned char *offsetBase = data + 6;
	unsigned int ifdOffset = readFourByteExifData(offsetBase + 0x04, bigEndian);
	if (ifdOffset + 12 > len) return;
	unsigned int tagCount = readTwoByteExifData(offsetBase + ifdOffset, bigEndian);
	if (tagCount > 300) return;	// Nobody should have 300 tags
	unsigned int tagOffset = ifdOffset + 2;
	if (tagOffset + tagCount * 12 > len) return;
	for (int tagIndex = 0; tagIndex < tagCount; ++tagIndex)
	{
		int tagNum = readTwoByteExifData(offsetBase + tagOffset, bigEndian);
		if (tagNum == 0x0112)
		{
			unsigned int orientation = readTwoByteExifData(offsetBase + tagOffset + 8, bigEndian);
			if (orientation >= 1 && orientation <= 8)
			{
				image->polyAllocated = orientation;
			}
			return;
		}
		tagOffset += 12;
...
void reinsert_exif_orientation_tag(j_compress_ptr cinfo, gdImagePtr image) 
{
	int orientation = image->polyAllocated;
	if (orientation < 1 || orientation > 8) return;
	unsigned char mimimalExifData[] = { 0x45, 0x78, 0x69, 0x66, 0x00, 0x00, // "Exif" and 2 zeros
			0x4D, 0x4D, 0x00, 0x2A, 0x00, 0x00, 0x00, 0x08,					// TIFF header: "MM", 42, offset of first IFD (8)
			0x00, 0x01, 																		// we have 1 tag entry
			0x01, 0x12, 0x00, 0x03, 0x00, 0x00, 0x00, 0x01, 0x00, orientation, 0x00, 0x00,		// TIFF tag record
			0x00, 0x00, 0x00, 0x00 };															// Next IFD offset (none)
	jpeg_write_marker(cinfo, JPEG_APP0 + 1, (unsigned char *) mimimalExifData, (unsigned int)sizeof(mimimalExifData));
}
// RCF: added fns to here
...
*/
// RCF: Renamed this to prevent collision with version in library
BGD_DECLARE(void *) rcf_gdImageJpegPtr(gdImagePtr im, int *size, int quality)
{
	void *rv;
...
static int _gdImageJpegCtx(gdImagePtr im, gdIOCtx *outfile, int quality)
{
	...
	// RCF: If we have saved an orientation, re-insert it into the output jpeg as we write it. 
	reinsert_exif_orientation_tag(&cinfo, im);
...
/*
  ...
  See <gdImageCreateFromJpegEx>.
*/
// RCF: Renamed this to prevent collision with version in gd library
BGD_DECLARE(gdImagePtr) rcf_gdImageCreateFromJpegPtr(int size, void *data)
{
...
// RCF: Save Exif marker block; we'll need it soon.
	jpeg_save_markers(&cinfo, JPEG_APP0 + 1, 65533);
...
// RCF: Look for an orientation tag in the exif data; save it for later reinsertion
	process_exif_markers(&cinfo, im);
