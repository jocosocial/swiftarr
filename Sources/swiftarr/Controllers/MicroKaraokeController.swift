...
struct MicroKaraokeController: APIRouteCollection {
	...
	func registerRoutes(_ app: Application) throws {
		...
		let modAuthGroup = addTokenCacheAuthGroup(to: baseRoute).grouped([RequireModeratorMiddleware()])
		modAuthGroup.get("mod", "songlist", use: getFullSongList)
		modAuthGroup.get("mod", "song", mkSongIDParam, use: getSongInfo)
		modAuthGroup.get("mod", "snippets", mkSongIDParam, use: getSnippetsForModeration)
		modAuthGroup.post("mod", "snippet", mkSnippetIDParam, "delete", use: deleteSnippet)
		modAuthGroup.delete("mod", "snippet", mkSnippetIDParam, use: deleteSnippet)
		modAuthGroup.post("mod", "approve", mkSongIDParam, use: approveSong)
...
// MARK: Moderation
	/// `GET /api/v3/microkaraoke/mod/songlist`
	///
	...
	/// `GET /api/v3/microkaraoke/mod/song/:song_id`
	///
	...
	/// `GET /api/v3/microkaraoke/mod/snippets/:song_id`
	///
	///  
	///
	/// - Parameter song_id: The song to get a manifest for.
	...
	/// `POST /api/v3/microkaraoke/mod/snippet/:snippet_id/delete`
	/// `DELETE /api/v3/microkaraoke/mod/snippet/:snippet_id/`
	///
	...
	/// `POST /api/v3/microkaraoke/mod/approve/:song_id`
	///
...
//		let totalSnippets = 3
		let krakenUserID = req.userCache.getUser(username: "kraken")?.userID ?? UUID()
		let newSongModel = MKSong(name: songInfo.songname, artist: songInfo.artist, totalSnippets: totalSnippets, 
				bpm: songInfo.bpm, krakenUserID: krakenUserID)
